Feature: V2 scan data
  ...
  Scenario: Valid V2 scan data for an existing service is updated in the database
    ...
    # The same (ip, port, service) attributes, but timestamp and response are different this time
    When a scan message with the following payload is published to the "scan-topic" topic:
      """
      {
        "ip": "3.3.3.3",
        "port": 21,
        "service": "FTP",
        "timestamp": 1732780226,
        "data_version": 2,
        "data": {
          "response_str": "foo bar"
        }
      }
      """
    Then the following data can be found in the "miniscan.scans" table after 3 seconds:
      | ip       | port | service | timestamp   | response    |
      | 3.3.3.3  | 21   | FTP     | 1732780226  | foo bar     |
